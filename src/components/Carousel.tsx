	...
	return (
		<>
			<div className="group relative h-full overflow-hidden rounded transition-all">
				<div className="relative flex w-full overflow-hidden">
					{images.map((image: any, index: number) => (
						<div
							onClick={handleOpen}
							key={index}
							className="max-h-[490px] min-w-full cursor-pointer transition-all"
				...
				<div className="pointer-events-none absolute top-0 flex w-full items-center justify-between bg-gradient-to-b from-black/75 to-transparent px-5 py-2 text-right text-white transition-all lg:-translate-y-full lg:group-hover:translate-y-0">
					<div>{distroName}</div>
					<div>
